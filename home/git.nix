{ ... }:
{
  ...
  programs.git = {
    ...
    aliases = {
      ...
      #   switching branches or when I need to stop work midway through a
      #   large change and I'd like to push unfinished changes to origin.
      #
      #   I only ever use it on personal "feature" branches, and use the
      #   "uncommit" alias when I resume work to ensure they don't stay in the
      #   history.
      ...
      # p = push
      #
      #   Pushes and sets the upstream to a branch with the same name. I use
      #   this universally for new and existing branches as I almost never
      #   push to a remote branch with a different name.
      #
      #   I haven't yet been able to find a simple way to do this once when a
      #   new branch is created. You can't use "git branch --set-upstream-to"
      #   unless the branch already exists, so I just set the upstream
      ...
      #   master, for example. --force-with-lease is a marginally safer form
      #   of --force that verifies no changes have occurred on the remote
      #   since you last did a pull/fetch. This will probably stop you blowing
      #   away somebody else changes, but wont stop you destroying your own!
      #   In practice I almost never force push to any branch that is used by
      #   other people.
      ...
      #   hash (#), which is unfortunately also the start of a markdown
      #   heading.
      #
      #   To get around this, I use --cleanup=whitespace, which stops Git from
      #   treating hash-prefixed lines as comments. By default Git includes
      #   some hash-prefixed lines in the commit message, which have to be
      #   manually removed. Setting an empty message and passing --edit
      #   prevent these lines from being added.
      #
      ...
      #
      #   Starts an interactive rebase that includes the commits on the
      #   current branch since it diverged from the default branch.
      rbi = "!git rebase --interactive $(git diverge-commit)";

      # swm = switch to main
      #
      #   It actually switches to the default branch, it doesn't have to be
      #   called "main", but this alias is now in my muscle memory and in
      #   practice the default branch is almost always "main".
